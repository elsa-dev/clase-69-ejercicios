...
const multiplicar = numbers2.reduce( (acc, valA)=> {
    return acc * valA
 })
...
const promedio = notasDeTPs.reduce( (acc, valA,) => {
    // return (acc + valA) / notasDeTPs.length
    return acc + valA / notasDeTPs.length
},0)
...
const libros = librosDeJS.reduce( ( acc, varActual) => {
    return  acc += `<li>${varActual}</li>`
}, " ")
...
const edades = personas.reduce( (acc, valActual) => {
    return    acc += valActual.edad
}, 0)
...
// const doble = positivos.map( ( simple) => {
//     return simple*2
// })

// console.log(doble)

// const total = doble.reduce( (acc, curr) => {
//     return acc+= curr
// },0)
